...
def handle_message(data):
    ...
    try:
        ...
        if message_data['type'] == 'text':
            ...
            print(f"🗣️ 用户输入：{get_str}, 语速：{speed}")
            ...
            # 生成语音
            audio = tts.tts_generate(dog_response['response'], dog_response['emotion'], speed)
            # 转WAV字节流
        ...
        # TTS Only 部分（不经过LLM）
        # ========================
        if message_data['type'] == 'tts_only':
            get_str = message_data['content']
            emotion_en = message_data.get('mood', 'neutral')
            speed = message_data.get('speed', 1.0)
            print(f"🗣️ TTS Only 输入：{get_str}, 情绪：{emotion_en}, 语速：{speed}")
            ...
            emotion_cn = emotion_cn_map.get(emotion_en, '其他')

            # 生成语音
            audio = tts.tts_generate(get_str, emotion_cn, speed)
            # 转WAV字节流
            wav_bytes = VPlay.audio_get_wav(audio)
            # 编码发送
            wav_base64 = base64.b64encode(wav_bytes).decode('utf-8')
            ...
            if emotion_en == "angry":
                face_id = "1"
                mood_val = "angry"
            elif emotion_en == "happy":
                face_id = "2"
                mood_val = "happy"
            elif emotion_en == "excited":
                face_id = "3"
                mood_val = "excited"
            elif emotion_en == "confused":
                face_id = "4"
                mood_val = "confused"
            else:
                face_id = "5"
            ...
            sio.emit('message', json.dumps(emotion_msg))
            print(f'🔊 TTS音频已发送, 情绪: {emotion_cn}')
